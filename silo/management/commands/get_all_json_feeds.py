...
from hikaya.util import importJSON
        ...
        for silo in silos:
            ...
            for read in reads:
                ...
                if result[0] == "error" or result[0] == 40:
                    ...
                    send_mail("Tola-Tables Auto-Pull Failed", "table_id: %s, source_id: %s, %s %s" % (silo.pk, read.pk, result[2], result[1]), settings.NOTIFICATION_SENDER, [silo.owner.email], fail_silently=False)
