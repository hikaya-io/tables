...
from silo.api import (CountryViewSet, CustomFormViewSet, OrganizationViewSet,
                      PublicSiloViewSet, ReadTypeViewSet, ReadViewSet,
...
urlpatterns = [
    ...
    url(r'^import_onedrive/(?P<id>\d+)/$', views.oneDriveImport,
        name='import_onedrive'),
    ...
    url(r'^provider_logout/(?P<provider>\w+)/$', views.providerLogout,
        name='providerLogout'),
    url(r'^saveAndImportRead/$', views.saveAndImportRead,
        name='saveAndImportRead'),
    url(r'^toggle_silo_publicity/$', views.toggle_silo_publicity,
        name='toggle_silo_publicity'),

    url(r'^silos', views.list_silos, name='list_silos'),
    url(r'^silo_detail/(?P<silo_id>\w+)/$', views.silo_detail,
        name='silo_detail'),
    url(r'^silo_edit/(?P<id>\w+)/$', views.edit_silo, name='edit_silo'),
    url(r'^silo_delete/(?P<id>\w+)/$', views.deleteSilo,
        name='deleteSilo'),
    url(r'^add_unique_fields', views.addUniqueFiledsToSilo,
        name='add_unique_fields_to_silo'),
    url(r'^anonymize_silo/(?P<id>\w+)/$', views.anonymizeTable,
        name='anonymize_table'),
    url(r'^identifyPII/(?P<silo_id>\w+)/$', views.identifyPII,
        name='identifyPII'),
    url(r'^source_remove/(?P<silo_id>\w+)/(?P<read_id>\w+)/$',
        views.removeSource, name='removeSource'),
    ...
    url(r'^updateMergedTable/(?P<pk>\w+)/$', views.updateSiloData,
        name='updateMergedTable'),
    ...
    url(r'^new_formula_column/(?P<pk>\w+)/$', views.newFormulaColumn,
        name='newFormulaColumn'),
    url(r'^edit_filter/(?P<pk>\w+)/$', views.addColumnFilter,
        name='editColumnFilter'),
    url(r'^edit_columns/(?P<id>\w+)/$', views.edit_columns,
        name='editColumns'),
    url(r'^delete_column/(?P<id>\w+)/(?P<column>\w+)/$', views.deleteColumn,
        name='deleteColumn'),
    url(r'^edit_column_order/(?P<pk>\w+)/$', views.editColumnOrder,
        name='editColumnOrder'),
    url(r'^set_column_type/(?P<pk>\w+)/$', views.setColumnType,
        name='setColumnType'),

    url(r'^export_silo_form/(?P<id>\w+)/$', views.export_silo_form,
        name='export_silo_form'),
    ...
    url(r'^store_oauth2_credential/$', gviews_v4.store_oauth2_credential,
        name='store_oauth2_credential'),
    url(r'^import_gsheet/(?P<id>\d+)/$', gviews_v4.import_from_gsheet,
        name='import_gsheet'),
    url(r'^get_sheets_from_google_spreadsheet/$',
    ...
    url(r'^renew_auto/(?P<read_pk>\d+)/(?P<operation>(pull|push))/$',
